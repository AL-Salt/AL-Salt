<?php 
    include_once("include/header.php");
?>
    <section class="map_section container my-5">

        <section class="">
            <div class="container d-flex justify-content-end text-left py-5">
                <div class="row">
                    <!-- <div class="col-12 col-sm-12 p-4">
                        <div class="bg-warning divedit_warning"></div>
                    </div> -->
                    <div class="card col-12 col-sm-12" style="width: 18rem; border: none; background-color:rgb(255,255,255,0.0);">
                        <img src="img/unnamed (1).png" class="card-img-top w-100 imgnewedit2020"alt="...">
                        <div class="card-body">
                            <h5 class="card-title">سشنت يشنستي </h5>
                            <p class="card-text">سشىسي نشس ينتشس ينتشس</p>
                            <!-- <a href="#" class="">Go somewhere</a> -->
                            <!-- Button trigger modal -->
                            <button type="button" class="btn_new_2020" data-toggle="modal" data-target="#exampleModal">
                                اقرا المزيد
                            </button>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Modal -->
            <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <button type="button" class="btn btn-warning" style="border:none" data-dismiss="modal" aria-label="Close">
                                <span style="color:white;font-size: 20px;" aria-hidden="true">&times;</span>
                            </button>
                            <!-- <h5 class="modal-title" id="exampleModalLabel">Modal title</h5> -->
                        </div>
                        <div class="modal-body text-left w-100" style="overflow: auto; height: 700px;">
                            <!-- MODEL -->
                            <div class="row">
                                <!-- <div class="col-12 col-sm-12  p-4">
                                    <div class="bg-warning divedit_warning"></div>
                                </div> -->
                                <div class="card col-12 col-sm-12" style="width: 18rem; border: none; background-color:rgb(255,255,255,0.0);">
                                    <img src="img/unnamed (1).png" class="card-img-top w-100 imgnewedit2020" alt="...">
                                    <div class="card-body">
                                        <h5 class="card-title">سشني شسنتي ش</h5>
                                        <p class="card-text">
                                            نسيى نسيىبنتسي بنت سينتب سينتب                                        </p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>

        <hr style="border: 0.5px solid gray;">

        <section class="">
            <div class="container d-flex justify-content-end text-left py-5">
                <div class="row">
                    <!-- <div class="col-12 col-sm-12  p-4">
                        <div class="bg-warning divedit_warning"></div>
                    </div> -->
                    <div class="card col-12 col-sm-12" style="width: 18rem; border: none; background-color:rgb(255,255,255,0.0);">
                        <img src="img/unnamed (1).png" class="card-img-top w-100 imgnewedit2020" alt="...">
                        <div class="card-body">
                            <h5 class="card-title">سيشيشس ينتشس </h5>
                            <p class="card-text">
                        سشية شسنتي تشسي تنشس ينتشسي نتشس ينت     
                        </p>
                            <!-- <a href="#" class="">Go somewhere</a> -->
                            <!-- Button trigger modal -->
                            <button type="button" class="btn_new_2020" style="border: none;" data-toggle="modal" data-target="#exampleModal1">
                            اقرا المزيد
                            </button>

                        </div>
                    </div>
                </div>
            </div>

            <!-- Modal -->
            <div class="modal fade" id="exampleModal1" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <button type="button" class="btn btn-warning" style="border:none" data-dismiss="modal" aria-label="Close">
                                <span style="color:white;font-size: 20px;" aria-hidden="true">&times;</span>
                            </button>
                            <!-- <h5 class="modal-title" id="exampleModalLabel">Modal title</h5> -->
                        </div>
                        <div class="modal-body text-left w-100" style="overflow: auto; height: 700px;">
                            <!-- MODEL -->
                            <div class="row">
                                <div class="col-12 col-sm-12  p-4">
                                    <div class="bg-warning divedit_warning"></div>
                                </div>
                                <div class="card col-12 col-sm-12" style="width: 18rem; border: none; background-color:rgb(255,255,255,0.0);">
                                    <img src="img/unnamed (1).png" class="card-img-top w-100 imgnewedit2020" alt="...">
                                    <div class="card-body">
                                        <h5 class="card-title">شسي نتشستي نتسش</h5>
                                        <p class="card-text">
                                            س تنيشسنتي نتشس ينتشس ينتشسنتي شسنتي نتشس يتشسينت شسنتي شسنتي نشتسي شستي نتشس  ي
                                        </p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>


        <hr style="border: 0.5px solid gray;">

        <section class="">
            <div class="container d-flex justify-content-end text-left py-5">
                <div class="row">
                    <!-- <div class="col-12 col-sm-12  p-4">
                        <div class="bg-warning divedit_warning"></div>
                    </div> -->
                    <div class="card col-12 col-sm-12" style="width: 18rem; border: none; background-color:rgb(255,255,255,0.0);">
                        <img src="img/unnamed (1).png" class="card-img-top w-100 imgnewedit2020" alt="...">
                        <div class="card-body">
                            <h5 class="card-title">سى يىشسي</h5>
                            <p class="card-text"> يب ي بسيبنتت يسبتن تن</p>
                            <!-- <a href="#" class="">Go somewhere</a> -->
                            <!-- Button trigger modal -->
                            <button type="button" class="btn_new_2020" style="border: none;" data-toggle="modal" data-target="#exampleModal">
                                اقرا المزيد
                            </button>

                        </div>
                    </div>
                </div>
            </div>

            <!-- Modal -->
            <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <button type="button" class="btn btn-warning" style="border:none" data-dismiss="modal" aria-label="Close">
                                <span style="color:white;font-size: 20px;" aria-hidden="true">&times;</span>
                            </button>
                            <!-- <h5 class="modal-title" id="exampleModalLabel">Modal title</h5> -->
                        </div>
                        <div class="modal-body text-left w-100" style="overflow: auto; height: 700px;">
                            <!-- MODEL -->
                            <div class="row">
                                <div class="col-12 col-sm-12  p-4">
                                    <div class="bg-warning divedit_warning"></div>
                                </div>
                                <div class="card col-12 col-sm-12" style="width: 18rem; border: none; background-color:rgb(255,255,255,0.0);">
                                    <img src="img/unnamed (1).png" class="card-img-top w-100 imgnewedit2020" alt="...">
                                    <div class="card-body">
                                        <h5 class="card-title">شسىش شسي</h5>
                                        <p class="card-text">
                                            تسشلاي نتسينتشستنين 
                                        </p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
</section>

<?php
    include_once("include/footer.php");
?>